...
function onBoardTitleClick(boardId) {
  ...
  const boardTitleElement = boardContainer.querySelector(".board-title");
...
function onAddTask(boardId, newTaskName) {
  ...
  if (board.tasks.length === 0) {
    task = new Task(1, newTaskName, false);
    board.tasks.push(task);
  } else {
    const lastTaskId = board.tasks[board.tasks.length - 1].id;
    task = new Task(lastTaskId + 1, newTaskName, false);
    board.tasks.push(task);
  }
  
  const boardContainer = document.querySelector(`[data-board-id="${boardId}"]`);
  const tasksContainer = boardContainer.querySelector(".tasks");
